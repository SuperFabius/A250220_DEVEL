...
S260320-R260820_DEVEL2 - HW ref: A250220 (V20-MBC)
...
// ------------------------------------------------------------------------------
//
// V20 CPU opcodes and IRQ vectors definitions
//
// ------------------------------------------------------------------------------
...
#define   SERIALRXIRQ_VECTOR    0x40            // Set the IRQ vector for the Serial Rx event   ********************* NEW (MAY BE CHANGED)
#define   SYSTICKIRQ_VECTOR     0x41            // Set the IRQ vector for the Sysytick event    ********************* NEW (MAY BE CHANGED)
...
const byte    sysTickTime  = 100;         // Period in milliseconds of the V20 Systick interrupt (if enabled) ***************
...
byte          V20IntRxEn = 0;             // IRQ enable on serial Rx flag                           ******************************
byte          V20IntTickEn = 0;           // IRQ enable on system tick timer (Systick)              ******************************
byte          V20IntTickIRQ = 0;          // Status (active/not active) of the Systick IRQ          ******************************
...
// ------------------------------------------------------------------------------
  ...
  Serial.begin(115200);
  Serial.print(F("\r\n\nV20-MBC - A250220\r\nIOS - I/O Subsystem - S260320-R260820_DEVEL2\r\n"));
  Serial.println();
...
void loop()
{
    ...
    
    {
     ...
     
      {
...

        {
          ...
          // NOTE: the order of the "if" statament defines the IRQ priority.
          // ************************************************************************************** NEW
          if (V20IntTickIRQ)
          // Systick IRQ
          {
            V20IntTickIRQ = 0;                  // Reset the Systick IRQ status to not active
            ioData = SYSTICKIRQ_VECTOR;         // Set the Systick IRQ vector
          }
          else
          // Serial Rx IRQ
          {
            ioData = SERIALRXIRQ_VECTOR;        // Set the serial Rx IRQ vector
          }
  ...
  if (Serial.available() && V20IntRxEn) digitalWrite(INTR, HIGH);  // Generate an IRQ on Serial Rx (if enabled)

  // ************************************************************************************************* NEW
  if (V20IntTickEn)
  // Systick IRQ is enabled. Check if the INTR signal must be activated
  {
    if ((millis() - timeStamp) > sysTickTime)
    // <sysTickTime> milliseconds are elapsed, so a Systick interrupt is required
    {
      digitalWrite(INTR, HIGH);                 // Generate the IRQ (INTR signal set active)
      V20IntTickIRQ = 1;                        // Set the Systick IRQ status to active
...
// ********************************************************************************************** NEW
void readRTCDOS(byte *second, byte *minute, byte *hour, uint16_t *since1980)
// Read current date/time binary values for MSDOS. Date is given as numbers of days since 1980. Author: Kevin Price
{
  byte    day = 0;
  byte    month = 0;
  byte    year = 0;                             // 0 to 99
  byte    temp = 0;
  *since1980 = 7304;                            // Days from 1980 to 2000 
  readRTC(second, minute, hour, &day, &month, &year, &temp);
...
void execWriteOpcode()
{
  ...
  // Execute the requested I/O WRITE Opcode. The 0xFF value is reserved as "No operation".
  {
    ...
    case  0x02:
      // ********************************************************************************************** CHANGED
      // SET IRQ:
      // Set or reset the flag to enable/disable the activation of the V20 INTR signal
      //
      //                I/O DATA:    D7 D6 D5 D4 D3 D2 D1 D0
      //                            ---------------------------------------------------------
      //                              x  x  x  x  x  x  x  0    Serial Rx IRQ off
      //                              x  x  x  x  x  x  x  1    Serial Rx IRQ on
      ...
      if (ioData & 1) V20IntRxEn = 1;           // Enable the IRQ on serial Rx
      if (ioData & 1<<1) V20IntTickEn = 1;      // Enable the IRQ on Systick timer
      //V20IntRxEn = ioData & 1;
      // ********************************************************************************************** END
    break;
...
void execReadOpcode()
{
  ...
  // Execute the requested I/O READ Opcode. The 0xFF value is reserved as "No operation".
  {
    ...
    // ******************************************************************************************************** NEW
    case  0x90:
      // MSDOS_RTC - (Read date/time for MSDOS. Binary values. Author: Kevin Price): 
      //                I/O DATA:    D7 D6 D5 D4 D3 D2 D1 D0
      //                            -----------------------------------------------------------------------------
      //                I/O DATA 0   D7 D6 D5 D4 D3 D2 D1 D0    seconds               [0..59]     (1st data byte)
      //                I/O DATA 1   D7 D6 D5 D4 D3 D2 D1 D0    minutes               [0..59]
      //                I/O DATA 2   D7 D6 D5 D4 D3 D2 D1 D0    hours                 [0..23]
      //                I/O DATA 3   D7 D6 D5 D4 D3 D2 D1 D0    day since 1980 (LSB)  [0..255]
      //                I/O DATA 4   D7 D6 D5 D4 D3 D2 D1 D0    day since 1980 (MSB)  [0..255]    (5th data byte)
      //
      // NOTE 1: If RTC is not found all read values wil be = 0
      // NOTE 2: Overread data (more then 5 bytes read) will be = 0
      if (foundRTC)
      {
        
        if (ioByteCnt == 0) readRTCDOS(&seconds, &minutes, &hours, &daysSince1980); // Read from RTC
        if (ioByteCnt < 5)
        // Send date/time (binary values) to Z80 bus
        {
          switch (ioByteCnt)
          {
            ...
            case 1: ioData = minutes; break;
            case 2: ioData = hours; break;
            case 3: ioData = (uint8_t) daysSince1980; break; //low days
            case 4: ioData = (uint8_t) (daysSince1980 >> 8); //high days
          }
          ioByteCnt++;
        }
        else ioOpcode = 0xFF;                   // All done. Set ioOpcode = "No operation"
  ...
  if ((ioOpcode != 0x84) && (ioOpcode != 0x86) && (ioOpcode !=0x90)) 
  // All done for the single byte opcodes
    ioOpcode = 0xFF;                            //  Set ioOpcode = "No operation"
}
